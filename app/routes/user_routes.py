...
@router.post("/signup")
async def signup(user: UserCredentials, supabase: Client = Depends(get_supabase_admin)):  # Note: using admin client
    try:
        ...
        logger.info(f"User data added to table: {data_response}")
...
async def dashboard(request: Request, supabase: Client = Depends(get_supabase)):
    try:
        # Get session using tokens from request state
        ...
        if access_token and refresh_token:
            ...
            if session and session.user:
                user = session.user
                user_name = user.user_metadata.get('name') or user.email.split('@')[0]
        ...
        return RedirectResponse(url="/user/login-page", status_code=303)
...
async def google_signin(
    ...
    try:
        ...
        # Set auth cookies with proper settings for localhost
        response.set_cookie(
            "sb-access-token",
            auth_response.session.access_token,
            httponly=True,
...
@router.post("/logout")
async def logout(request: Request, supabase: Client = Depends(get_supabase)):
    try:
        supabase.auth.sign_out()
        response = RedirectResponse(url="/user/login-page", status_code=303)
...
# Optional: Add a callback route for PKCE flow if needed
...
async def auth_callback(request: Request, code: str = None):
    try:
        ...
        return RedirectResponse(url="/user/login-page?error=auth_failed")